...
func TestTopdepartmentLogic_Topdepartment(t *testing.T) {
	c := config.Config{}
	...
	tests := []struct {
		...
		ctx        context.Context
		setupMocks func()
		req        *types.TopDepartmentReq
		wantErr    bool
		checkResp  func(resp *types.TopDepartmentResp, err error)
	}{
		{
			name: "response error",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.setupMocks()
			l := NewTopdepartmentLogic(tt.ctx, mockSvcCtx)
			resp, err := l.Topdepartment(tt.req)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
