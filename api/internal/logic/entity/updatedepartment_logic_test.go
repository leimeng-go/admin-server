...
func TestUpdatedepartmentLogic_Updatedepartment(t *testing.T) {
	c := config.Config{}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tt.setupMocks()
			l := NewUpdatedepartmentLogic(tt.ctx, mockSvcCtx)
			err := l.Updatedepartment(tt.req)
			if tt.wantErr {
				assert.Error(t, err)
			} else {
